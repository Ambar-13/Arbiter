"""
ConstrAI — Layer 3: Hardening (v2)
=================================

Addresses 5 real vulnerabilities identified by audit:

1. COMMAND INJECTION        → Sandboxed Attestors (frozen commands)
2. TEMPORAL BLINDNESS       → Temporal Dependencies (readiness probes)
3. BAYESIAN COLD-START      → Cost-Aware Priors (pessimistic first-strike)
4. SPEC-REALITY GAP         → Environment Reconciliation (model vs reality)
5. REWARD HACKING           → Multi-Dimensional Attestation (cross-checks)

Plus retained from v1:
A. Active Attestation (goal verification)
B. Dynamic Dependency Discovery
C. Resource-Aware State

DESIGN PRINCIPLE: "We don't monitor the AI; we monitor the environment
the AI is touching. If the environment doesn't change how the math says
it should, the AI is stopped instantly."
"""
...
from .formal import State, ActionSpec, Invariant, GuaranteeLevel, Claim
from .reasoning import BeliefState, CausalGraph, Belief, Dependency


# ═══════════════════════════════════════════════════════════════════════════
# §1  FIX 1: SANDBOXED ATTESTORS (Command Injection Kill)
# ═══════════════════════════════════════════════════════════════════════════
...
class SubprocessAttestor:
    ...
    def __init__(self, name: str, command: List[str],
                 ...
                 command_allowlist: frozenset = None):
        if not command:
...
# ═══════════════════════════════════════════════════════════════════════════
# §2  FIX 2: TEMPORAL DEPENDENCIES (Readiness Probes)
# ═══════════════════════════════════════════════════════════════════════════
...
class TemporalCausalGraph:
    ...
    def mark_completed(self, action_id: str, at_time: float = None) -> None:
...
# ═══════════════════════════════════════════════════════════════════════════
# §3  FIX 3: COST-AWARE BAYESIAN PRIORS (Cold-Start Kill)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# §4  FIX 4: ENVIRONMENT RECONCILIATION (Spec-Reality Gap)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# §5  FIX 5: MULTI-DIMENSIONAL ATTESTATION (Reward Hack Kill)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# §6  RETAINED: Dynamic Dependency Discovery
# ═══════════════════════════════════════════════════════════════════════════
...
class DependencyDiscovery:
    ...
    def _chi2p(p: FailurePattern) -> float:
        ...
        for t, p in [(10.83,.001),(6.63,.01),(5.02,.025),(3.84,.05),
...
# ═══════════════════════════════════════════════════════════════════════════
# §7  RETAINED: Resource-Aware State
# ═══════════════════════════════════════════════════════════════════════════
...
class ResourceTracker:
    ...
    def __init__(self): self._resources: Dict[str, ResourceDescriptor] = {}
    def register(self, r: ResourceDescriptor): self._resources[r.identifier] = r
    ...
    def all_ready(self, ids: List[str] = None) -> bool:
...
HARDENING_CLAIMS = [
    ...
    Claim("H5", "Multi-dim attestation harder to game than single-metric",
          ...
          "More constraints = harder to satisfy simultaneously."),
