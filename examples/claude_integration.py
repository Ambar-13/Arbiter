"""
ConstrAI + Claude Integration Example
====================================

This example shows how to plug a real LLM (Claude) into ConstrAI
for actual autonomous task execution with full safety guarantees.
...
from constrai import (
    ...
    LLMAdapter, ConstrAI_SYSTEM_PROMPT,
)


# ═══════════════════════════════════════════════════════════════════════════
# §1  CLAUDE ADAPTER — Plugs Anthropic API into ConstrAI
# ═══════════════════════════════════════════════════════════════════════════
...
class ClaudeAdapter:
    ...
    Implements the LLMAdapter protocol:
      complete(prompt, system_prompt, temperature, max_tokens) -> str
    
    That's it. ConstrAI handles all structured parsing, validation,
...
if __name__ == "__main__":
    ...
    print("\n🔒 Safety Verification:")
    print(f"  T1 Budget:     {'✓' if result.total_cost <= task.budget else '✗'} "
          f"(${result.total_cost:.2f} ≤ ${task.budget:.2f})")
    print(f"  T2 Terminated: ✓")
    print(f"  T3 Invariants: {'✓' if all(i.violation_count == 0 for i in task.invariants) else '✗'}")
    ok, msg = engine.kernel.trace.verify_integrity()
    print(f"  T6 Trace:      {'✓' if ok else '✗'} {msg}")
