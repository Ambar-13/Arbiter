...
# Run main test suite first
exec(open(os.path.join(os.path.dirname(__file__), "test_constrai.py")).read())

# T is inherited from test_constrai.py
...
for trial in range(N_TRIALS):
    ...
    all_inv_ok = all(
        inv.check(result.final_state)[0] for inv in task.invariants
    )
    if all_inv_ok:
