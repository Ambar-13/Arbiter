"""
ConstrAI Chaos Fuzzer
===================

A systematic adversarial test suite that tries to BREAK ConstrAI.

Attack vectors:
  F1  Hallucinated Actions:    LLM proposes actions that don't exist
  F2  Budget Overflow:         Try to spend more than allowed
  F3  Invariant Evasion:       Try to reach forbidden states
  F4  State Poisoning:         Try to mutate state externally
  F5  Trace Tampering:         Try to alter execution history
  F6  Success Cheating:        Try to claim goal without doing work
  F7  Resource Exhaustion:     Try to create infinite loops
  F8  Malicious LLM Gallery:   Multiple attack LLM personalities
  F9  Dependency Bypass:       Try to skip required steps
  F10 Concurrent Mutation:     Try to mutate state during evaluation
  F11 Type Confusion:          Pass wrong types to everything
  F12 Dynamic Discovery Poisoning: Feed false failure patterns
...
from constrai import (
    ...
    SafetyKernel, BudgetController, ExecutionTrace,
    Belief, BeliefState, CausalGraph,
    TaskDefinition, Orchestrator, TerminationReason,
    parse_llm_response, MockLLMAdapter,
)
from constrai.hardening import (
    AttestationGate, PredicateAttestor, AttestationResult,
    DependencyDiscovery, ResourceTracker, ResourceDescriptor,
    ResourceState, Permission,
)
...
class FuzzResults:
    ...
    def attack(self, name: str, blocked: bool, detail: str = ""):
        status = "BLOCKED" if blocked else "⚠ BREACHED"
    ...
    def summary(self):
        ...
        if self.breached > 0:
            ...
            print(f"  ✅ ALL ATTACKS BLOCKED — Framework held.")
        print(f"{'='*60}")
...
# ═══════════════════════════════════════════════════════════════════════════
# F6: SUCCESS CHEATING (Attestation Test)
# ═══════════════════════════════════════════════════════════════════════════
...
goal_pred = lambda s: s.get("file_created", False)
...
# ═══════════════════════════════════════════════════════════════════════════
# F13: STATISTICAL FUZZING (1000 random attacks)
# ═══════════════════════════════════════════════════════════════════════════
